...
// node_modules/@babylonjs/core/Animations/animation.pure.js
...
function AnimationMakeAnimationAdditive(sourceAnimation, referenceFrameOrOptions, range, cloneOriginal = false, clonedName) {
  let options;
...
function AnimationCreateAnimation(property, animationType, framePerSecond, easingFunction) {
...
function AnimationCreateAndStartAnimation(name, target, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction, onAnimationEnd, scene) {
...
function AnimationCreateAndStartHierarchyAnimation(name, node, directDescendantsOnly, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction, onAnimationEnd) {
  const animation = Animation._PrepareAnimation(name, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction);
  if (!animation) {
    return null;
  }
...
function AnimationCreateMergeAndStartAnimation(name, node, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction, onAnimationEnd) {
  const animation = Animation._PrepareAnimation(name, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction);
  if (!animation) {
    return null;
  }
...
function AnimationTransitionTo(property, targetValue, host, scene, frameRate, transition, duration, onAnimationEnd = null, stopCurrent = true, customKeys) {
  if (duration <= 0) {
  ...
  const animatable = scene.beginAnimation(host, 0, endFrame, false, 1, onAnimationEnd ?? void 0, void 0, stopCurrent);
  return animatable;
}
function AnimationAppendSerializedAnimations(source, destination) {
  SerializationHelper.AppendSerializedAnimations(source, destination);
}
function AnimationParse(parsedAnimation) {
...
async function AnimationParseFromFileAsync(name, url) {
...
async function AnimationParseFromSnippetAsync(snippetId) {
...
var AnimationCreateFromSnippetAsync = AnimationParseFromSnippetAsync;
...
export {
  ...
  AnimationParse,
  AnimationParseFromFileAsync,
  AnimationParseFromSnippetAsync,
  AnimationCreateFromSnippetAsync,
  RegisterAnimation
};
//# sourceMappingURL=chunk-Y7ZT2VK7.js.map
