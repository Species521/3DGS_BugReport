...
} from "./chunk-AS5FMVVA.js";
import "./chunk-YCGOGKUP.js";
import {
  AssetContainer
} from "./chunk-UOX7FS34.js";
import {
...
import "./chunk-OPO3GRXL.js";
import "./chunk-JPJXMON3.js";
...
import "./chunk-SXWIXRKO.js";
import "./chunk-XYON5HHY.js";
import "./chunk-6HF3G62K.js";
import "./chunk-NNLIP7GC.js";
import "./chunk-CBB4SEM4.js";
...
import "./chunk-KCG2UG3W.js";
import "./chunk-63DO6QNO.js";
import "./chunk-R35S4W26.js";
...
import "./chunk-DU63Z3LB.js";
import {
  Color4
} from "./chunk-4UCP53KD.js";
import "./chunk-ZPPTIOZU.js";
...
var _SpzConversionBatchSize = 32768;
var _SH_C0 = 0.28209479177387814;
var _SpzModulePromise = null;
var _SpzModuleUrl = null;
function ParseSpz(data, scene, _loadingOptions) {
  ...
  if (reserved || ubufu32[0] != 1347635022 || version < 2 || version > 4) {
    return new Promise((resolve) => {
      resolve({ mode: 3, data: new ArrayBuffer(0), hasVertexColors: false });
    });
  }
  ...
  if (shDegree) {
    ...
    return new Promise((resolve) => {
      resolve({ mode: 0, data: buffer, hasVertexColors: false, sh, shDegree, trainedWithAntialiasing: !!flags });
    });
  }
  return new Promise((resolve) => {
...
function* ConvertSpzToSplat(cloud, scene, useCoroutine = false) {
  ...
  for (let i = 0; i < splatCount; i++) {
    ...
    uBuffer[uBase + 27] = 1 / (1 + Math.exp(-alphas[i])) * 255 + 0.5 | 0;
    ...
    uBuffer[uBase + 28] = rw <= 0 ? 0 : rw >= 255 ? 255 : rw + 0.5 | 0;
    uBuffer[uBase + 29] = rx <= 0 ? 0 : rx >= 255 ? 255 : rx + 0.5 | 0;
    uBuffer[uBase + 30] = ry <= 0 ? 0 : ry >= 255 ? 255 : ry + 0.5 | 0;
    uBuffer[uBase + 31] = rz <= 0 ? 0 : rz >= 255 ? 255 : rz + 0.5 | 0;
    if (cloudSh && shArrays && chunkStarts && chunkEnds) {
      ...
      for (let t = 0; t < shArrays.length; t++) {
        ...
        for (let j = chunkStart; j < chunkEnd; j++) {
          const v = cloudSh[shSplatBase + j] * 128 + 128;
  ...
  if (cloud.extensions) {
    for (const ext of cloud.extensions) {
      ...
      if (safeOrbitExt.safeOrbitRadiusMin !== void 0) {
  ...
  return {
    ...
    sh: sh !== null ? sh : void 0,
    shDegree: shDegree > 0 ? shDegree : void 0,
...
async function ParseSogDatas(data, imageDataArrays, scene) {
  ...
  if (data.shN) {
    const coeffs = data.shN.bands ? (data.shN.bands + 1) ** 2 - 1 : data.shN.shape[1] / 3;
    const shDegree = data.shN.bands !== void 0 && data.shN.bands !== null ? data.shN.bands : Math.round(Math.sqrt(coeffs + 1) - 1);
    ...
    return await new Promise((resolve) => {
      resolve({ mode: 0, data: buffer, hasVertexColors: false, sh, shDegree });
    });
  }
  return await new Promise((resolve) => {
...
var SPLATFileLoader = class _SPLATFileLoader {
  ...
  constructor(loadingOptions = {}) {
    ...
    this._loadingOptions = { ..._SPLATFileLoader._DefaultLoadingOptions, ...loadingOptions };
  }
  /** @internal */
  ...
  _parseAsync(meshesNames, scene, data, rootUrl) {
    ...
    const makeGSFromParsedSOG = (parsedSOG) => {
      ...
      const gaussianSplatting = this._loadingOptions.gaussianSplattingMesh ?? new GaussianSplattingMesh("GaussianSplatting", null, scene, this._loadingOptions.keepInRam, this._loadingOptions.needsRotationScaleTextures);
      gaussianSplatting._parentContainer = this._assetContainer;
      babylonMeshesArray.push(gaussianSplatting);
      gaussianSplatting.updateData(parsedSOG.data, parsedSOG.sh, { flipY: false }, void 0, parsedSOG.shDegree);
          ...
          case 0:
            {
              ...
              gaussianSplatting._parentContainer = this._assetContainer;
              babylonMeshesArray.push(gaussianSplatting);
              gaussianSplatting.updateData(parsedPLY.data, parsedPLY.sh, { flipY: false }, void 0, parsedPLY.shDegree);
              gaussianSplatting.scaling.y *= -1;
    ...
    const isGZipped = u8[0] === 31 && u8[1] === 139;
    const isNGSP = u8[0] === 78 && u8[1] === 71 && u8[2] === 83 && u8[3] === 80;
    if (!isGZipped && !isNGSP) {
      return new Promise((resolve) => {
        handlePLY(resolve);
      });
    ...
    const applyParsedSPZ = (parsedSPZ, resolve) => {
      ...
      gaussianSplatting._parentContainer = this._assetContainer;
      babylonMeshesArray.push(gaussianSplatting);
      gaussianSplatting.updateData(parsedSPZ.data, parsedSPZ.sh, { flipY: false }, void 0, parsedSPZ.shDegree);
    ...
    if (isNGSP) {
      return Promise.reject(new Error("SPZ V4+ files (NGSP format) are not supported by the native fallback loader. Please provide a valid 'spzLibraryUrl' in the loading options to use the WASM-based SPZ library, or ensure WebAssembly is available in your environment."));
    }
  ...
  applyAutoCameraLimits(meta, scene) {
    ...
    if ((meta.safeOrbitCameraRadiusMin !== void 0 || meta.safeOrbitCameraElevationMinMax !== void 0) && scene.activeCamera?.getClassName() === "ArcRotateCamera") {
...
SPLATFileLoader._DefaultLoadingOptions = {
  ...
  spzLibraryUrl: typeof WebAssembly === "object" ? "https://unpkg.com/@adobe/spz@0.2.2/dist/spz.js" : void 0
};
RegisterSceneLoaderPlugin(new SPLATFileLoader());
export {
  SPLATFileLoader
};
//# sourceMappingURL=@babylonjs_loaders_SPLAT_splatFileLoader.js.map
