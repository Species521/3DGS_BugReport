...
export async function createBabylonScene(options) {
    const lights = [];
    const meshes = [];
    const cameras = [];
    const materials = [];
    ...
    // Meshes
    const meshesResult = await Promise.all(options.directories.meshesFiles.map(async (file) => {
        ...
        if (data.lods) {
            mesh.lodMeshIds = [];
            mesh.lodDistances = [];
            const lodsResult = await Promise.all(data.lods?.map(async (lodFile) => {
    ...
    // Morph targets
    const morphTargetResult = await Promise.all(options.directories.morphTargetManagerFiles.map(async (file) => {
    ...
    // Transform nodes
    const transformNodesResult = await Promise.all(options.directories.nodesFiles.map(async (file) => {
    ...
    transformNodes.push(...transformNodesResult.filter((transformNode) => {
        return transformNode !== null;
    }));
    // Sprite managers
    const spriteManagersResult = await Promise.all(options.directories.spriteManagerFiles.map(async (file) => {
    ...
    transformNodes.push(...spriteManagersResult.filter((spriteManager) => {
        return spriteManager !== null;
    }));
    // Sprite maps
    const spriteMapsResult = await Promise.all(options.directories.spriteMapFiles.map(async (file) => {
    ...
    transformNodes.push(...spriteMapsResult.filter((spriteMap) => {
        return spriteMap !== null;
    }));
    // Sound nodes
    const soundNodesResults = await Promise.all(options.directories.soundNodeFiles.map(async (file) => {
    ...
    transformNodes.push(...soundNodesResults.filter((soundNode) => {
        return soundNode !== null;
    }));
    // Lights
    const lightsResult = await Promise.all(options.directories.lightsFiles.map(async (file) => {
    ...
    lights.push(...lightsResult.filter((light) => {
        return light !== null;
    }));
    // Cameras
    const camerasResult = await Promise.all(options.directories.cameraFiles.map(async (file) => {
    ...
    cameras.push(...camerasResult.filter((camera) => {
        return camera !== null;
    }));
    // Extract materials
    ...
    await Promise.all(materials.map(async (material) => {
        if (material.customType === "BABYLON.NodeMaterial") {
            ...
            await Promise.all(result.map(async (relativePath) => {
    ...
    const particleSystemsResult = await Promise.all(options.directories.particleSystemFiles.map(async (file) => {
    ...
    particleSystems.push(...particleSystemsResult.filter((ps) => {
        return ps !== null;
    }));
    // Extract node particle system sets
    const nodeParticleSystemSetsResult = await Promise.all(options.directories.nodeParticleSystemSetFiles.map(async (file) => {
    ...
    meshes.push(...nodeParticleSystemSetsResult.filter((node) => {
        return node !== null;
    }));
    // Shadow generators
    const shadowGeneratorsResult = await Promise.all(options.directories.shadowGeneratorFiles.map(async (file) => {
    ...
    shadowGenerators.push(...shadowGeneratorsResult.filter((sg) => {
        return sg !== null;
    }));
    // Skeletons
    const skeletonsResult = await Promise.all(options.directories.skeletonFiles.map(async (file) => {
    ...
    skeletons.push(...skeletonsResult.filter((skeleton) => {
    ...
    const scene = {
        ...
        meshes,
        particleSystems,
        sounds: await Promise.all(options.directories.soundFiles.map(async (file) => {
            return fs.readJSON(join(options.sceneFile, "sounds", file));
        })),
        shadowGenerators,
        animationGroups: await Promise.all(options.directories.animationGroupFiles.map(async (file) => {
    ...
    const geometries = await Promise.all(geometryFiles.map(async (file) => {
    ...
    for (let i = 0, iLen = geometries.length; i < iLen; ++i) {
        for (let j = i + 1, jLen = geometries.length; j < jLen; ++j) {
            if (isSameArray(geometries[i].data, geometries[j].data)) {
