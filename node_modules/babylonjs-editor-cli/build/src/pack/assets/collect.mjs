...
const collectedSupportedExtensions = [...supportedExtensions, binaryGeometryExtension];
export function traverseAndReplaceInSceneObject(scene, callback) {
    function recursivelyCollect(root) {
...
async function _checkKtxSupportForTexture(value, publicDir, result) {
...
export async function collectUsedAssetsForScene(scene, publicDir) {
    const result = [];
    let stringValues = [];
    function recursivelyCollect(root) {
        for (const thing in root) {
    ...
    await Promise.all(stringValues.map(async (value) => {
        const extension = extname(value).toLowerCase();
        ...
        if (await pathExists(absolutePath)) {
            ...
            if (supportedImagesExtensions.includes(extension)) {
                ...
                await Promise.all(availableSizes.map(async (size) => {
